...
install_skills_to() {
    ...
    local GEMINI_MD="$TARGET/GEMINI.md"
    local CLAUDE_MD_SRC="$SCRIPT_DIR/CLAUDE.md"
    local GEMINI_MD_SRC="$SCRIPT_DIR/GEMINI.md"

    if [ ! -d "$TARGET" ]; then
        echo "  SKIP $LABEL: $TARGET not found (install the CLI first)"
        return
    fi
    ...
    # Write config MD
    if [ "$LABEL" = "Gemini CLI" ] && [ -f "$GEMINI_MD_SRC" ]; then
        cp "$GEMINI_MD_SRC" "$GEMINI_MD"
        echo "  ✓ $LABEL: GEMINI.md written"
    elif [ "$LABEL" = "Claude Code" ] && [ -f "$CLAUDE_MD_SRC" ]; then
        # Append to CLAUDE.md if Flow Kit section not present
        GLOBAL_CLAUDE="$TARGET/CLAUDE.md"
        if [ -f "$GLOBAL_CLAUDE" ] && grep -q "Flow Kit" "$GLOBAL_CLAUDE"; then
            echo "  OK $LABEL: CLAUDE.md already has Flow Kit section"
        else
            cat "$CLAUDE_MD_SRC" >> "$GLOBAL_CLAUDE"
            echo "  ✓ $LABEL: Flow Kit appended to CLAUDE.md"
        fi
    fi

    # Install each skill
    COUNT=0
    for SKILL_FILE in "$SKILLS_SRC"/fk:*.md; do
        SKILL_NAME=$(basename "$SKILL_FILE" .md)
        SKILL_DIR="$SKILLS_DST/$SKILL_NAME"
        mkdir -p "$SKILL_DIR"
        FIRST_LINE=$(head -1 "$SKILL_FILE" | sed 's/^#* *//')
        printf -- "---\nname: %s\ndescription: %s\n---\n\n" "$SKILL_NAME" "$FIRST_LINE" > "$SKILL_DIR/SKILL.md"
...
install_skills_to "$HOME/.gemini" "Gemini CLI"
install_skills_to "$HOME/.claude" "Claude Code"
